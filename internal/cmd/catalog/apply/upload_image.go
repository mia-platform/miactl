...
const (
	uploadImageEndpointTemplate = "/api/tenants/%s/marketplace/files"
...
func getAndValidateImageLocalPath(item *catalog.Item, imageKey, imageURLKey string) (string, error) {
	...
	if imageExists {
		imageInfoObj, ok := imageInfo.(catalog.Item)
		if !ok {
...
func uploadSingleFileWithMultipart(
	...
	uploadResp := &catalog.UploadImageResponse{}
