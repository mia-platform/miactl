...
func removeUserTestServer(t *testing.T) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		params := r.URL.Query()
		switch {
		case r.Method == http.MethodDelete && r.URL.Path == fmt.Sprintf(removeUserRoleTemplate, "success", "000000000000000000000001"):
			assert.Equal(t, "true", params.Get(removeFromGroupParamKey))
			w.WriteHeader(http.StatusOK)
		case r.Method == http.MethodDelete && r.URL.Path == fmt.Sprintf(removeUserRoleTemplate, "only-iam", "000000000000000000000001"):
			assert.Equal(t, "false", params.Get(removeFromGroupParamKey))
			w.WriteHeader(http.StatusOK)
		case r.Method == http.MethodDelete && r.URL.Path == fmt.Sprintf(removeUserRoleTemplate, "fail", "000000000000000000000001"):
